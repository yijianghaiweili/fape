...
public:
    ...
    static void write(const std::string &file_path, std::vector<type> &vec);
    template <typename type>
    static void read(const std::string &file_path, std::vector<type> &vec);

    /**
     * Array IO.
     */
    template <typename type>
    static void write(const std::string &file_path, type *src, uint32_t length);
    template <typename type>
    static void read(const std::string &file_path, type *&src, uint32_t &length);
};
